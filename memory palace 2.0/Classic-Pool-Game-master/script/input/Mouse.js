...
function handleTouchStart(evt) {
    evt.preventDefault(); // Prevent mouse events from being fired
    handleTouchMove(evt);
    handleMousePress(1); // Treat touch as left mouse button press
}

function handleTouchEnd(evt) {
    handleTouchMove(evt);
    handleMouseRelease(1); // Treat touch as left mouse button release
}
...
function handleMouseRelease(button) {
    if (button === 1) Mouse._left.down = false;
    else if (button === 2) Mouse._middle.down = false;
    else if (button === 3) Mouse._right.down = false;
}
